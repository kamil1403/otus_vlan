...
Vagrant.configure("2") do |config|
  MACHINES.each do |boxname, boxconfig|
    config.vm.define boxname do |machine|
      machine.vm.box = boxconfig[:box_name]
      machine.vm.host_name = boxconfig[:vm_name]
      # Версию не указываем, подтянется 1.0.0 с зеркала
      ...
      boxconfig[:net].each do |ipconf|
        machine.vm.network "private_network", **ipconf
      end
